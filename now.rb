...
data[:location][:html] = "#{location['now']['country']} #{location['now']['country_code'].upcase.tr('A-Z', "\u{1F1E6}-\u{1F1FF}")}"
data[:location][:image] = "url('#{wikipedia.image_urls.first}')"

data[:next_location] = Hash.new
if !location['next']['city'].nil?
  data[:next_location][:html] = "Leaving for #{location['next']['country']} #{location['next']['country_code'].upcase.tr('A-Z', "\u{1F1E6}-\u{1F1FF}")} in #{(Date.parse(location['next']['date_start']) - Date.today).to_i} days"
else 
  data[:next_location][:html] = "With no plans after #{Date.parse(location['now']['date_end']).strftime("%B %e")}. Any recommendation of a nice city nearby? Drop me a Tweet"
end
# End of location

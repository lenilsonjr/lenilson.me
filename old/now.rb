...
data[:location][:html] = "#{location['now']['city']} #{location['now']['country_code'].upcase.tr('A-Z', "\u{1F1E6}-\u{1F1FF}")}"
data[:location][:image] = "url('#{wikipedia.image_urls.first}')"

data[:next_location] = Hash.new
if !location['next']['city'].nil?
  data[:next_location][:html] = "Leaving for #{location['next']['city']} #{location['next']['country_code'].upcase.tr('A-Z', "\u{1F1E6}-\u{1F1FF}")} in #{(Date.parse(location['next']['date_start']) - Date.today).to_i} days"
else 
